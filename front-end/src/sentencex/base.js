...
export default class Language {
  ...
  is_abbreviation(head, tail, seperator) {
    ...
    const isAbbrev =
      ...
      this.abbreviations.has(lastWord[0].toLowerCase() + lastWord.slice(1)) ||
  ...
  is_exclamation_word(head, _tail) {
  ...
  segment(text) {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      let match;
      while (
        (match = this.constructor.sentenceBreakRegex.exec(paragraph)) !== null
      ) {
        matches.push(match);
      }
